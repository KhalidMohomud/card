...
Production-oriented, single-store car wash point-of-sale and operations system built with Next.js 16, TypeScript, PostgreSQL, Prisma, Better Auth, Zod, and Tailwind CSS.

## Included modules

- Username/password authentication with secure Better Auth sessions, login throttling, account disable support, and ADMIN/SUPERVISOR authorization.
...
- `BETTER_AUTH_URL`: canonical application URL, such as `http://localhost:3000` locally or the production HTTPS URL.
- `ADMIN_NAME`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`: used by the idempotent seed to create/update the first administrator. The password must be at least 10 characters.
...
Open `http://localhost:3000` and sign in with the seeded administrator username and password. Immediately update the placeholder business details under Settings.
...
3. Add all environment variables listed above to Production. Set `BETTER_AUTH_URL` to the production HTTPS origin.
