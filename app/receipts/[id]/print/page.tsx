...
export default async function ReceiptPrintPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ autoprint?: string }> }) {
  ...
  return <main className="receipt-screen">
    <div className="print-actions no-print">
      <Link className="btn btn-ghost" href={user.role === "SUPERVISOR" ? "/pos" : "/receipts"}>
        <ArrowLeft size={17} /> {user.role === "SUPERVISOR" ? "Back to POS" : "Receipts"}</Link>
    ...
    <article className="receipt-paper">
      ...
      {business.phone && <div className="center">{business.phone}
      </div>
      }
      <div className="rule" />
      <div className="center">
        <strong>RECEIPT #{String(receipt.id).padStart(6, "0")}</strong>
      </div>
      <div className="center">{formatDateTime(receipt.issuedAt)}</div>
      {receipt.status === "CANCELLED" && <><div className="rule" />
        <div className="center">
      ...
      <div className="row">
        <span>Service</span>
        <span>{receipt.serviceNameSnapshot}</span>
      </div>
      <div className="row">
        <span>Supervisor</span>
        <span>{receipt.createdByUser.fullName}</span>
      </div>
      <div className="row">
        <span>Payment</span>
        <span>{receipt.paymentMethod.name}</span>
      </div>{receipt.paymentReference && <div className="row">
        <span>Reference</span>
      ...
      <div className="row" style={{ fontSize: 14 }}>
        <strong>TOTAL</strong>
        <strong>{formatMoney(receipt.servicePriceSnapshot, business.currencyCode)}</strong>
      </div>
      <div className="rule" />
      {receipt.printCount > 0 && <div className="center">REPRINT · COPY {receipt.printCount}
      </div>}
      <div className="center" style={{ marginTop: 10 }}>{business.receiptFooter}</div>
    </article>
  </main>;
}
