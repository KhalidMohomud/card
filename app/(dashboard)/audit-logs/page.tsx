...
export default async function AuditLogsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  await requireAdmin(); const query = await searchParams; const filters = auditLogFilterInput.parse(query); const ledger = await getAuditLedger(filters.action, filters.entity, filters.page); const logs = ledger.rows, total = ledger.total;
