...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className="page">
    <div className="page-head"><div><span className="eyebrow">Stock control</span><h1>Inventory overview</h1><p>Monitor live quantities calculated from permanent stock movements.</p></div><div className="actions"><InventoryAdjustmentDialog items={stock.map(({ id, name, sku, unit }) => ({ id, name, sku, unit }))} /><Link className="btn btn-soft" href="/inventory/issues"><Truck size={16} /> Issues & returns</Link><Link className="btn btn-primary" href="/purchases"><PackagePlus size={16} /> Register purchase</Link></div></div>
