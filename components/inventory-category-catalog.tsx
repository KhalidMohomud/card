...
import { FolderPlus, Pencil, Plus, Save, Search, Tags, Trash2, X } from "lucide-react";
...
function DeleteButton({ disabled, name }: { disabled: boolean; name: string }) { const { pending } = useFormStatus(); return <button className="btn btn-danger" disabled={disabled || pending} title={disabled ? "Archive categories that contain items" : undefined} onClick={(event) => { if (!window.confirm(`Permanently delete ${name}? This cannot be undone.`)) event.preventDefault(); }}><Trash2 size={15} />{pending ? "Deleting…" : "Delete category"}</button>; }
