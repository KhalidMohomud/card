...
export const LOGIN_IP_MAX_ATTEMPTS = 25;
...
export function nextLoginThrottle(current: ExistingThrottle | null, now: Date, maxAttempts = LOGIN_MAX_ATTEMPTS) {
  ...
  const blockedUntil = attempts >= maxAttempts ? new Date(now.getTime() + LOGIN_BLOCK_MS) : priorBlock;
